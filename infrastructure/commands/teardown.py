...
class Teardown(ScriptCommand):
    ...
    def destroy_jenkins_server(self) -> None:
        jenkins_machine = Terraform(working_dir='./development/')
        output = jenkins_machine.destroy(auto_approve=IsFlagged, force=IsNotFlagged)
        print(format_tf_output("Teardown of Jenkins Server", output))
